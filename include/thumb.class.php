...
class Thumb {
	...
	static function fixed($im, $w, $h, $Type) {
		...
		$scale = min($w/$im_w, $h/$im_h); //求出绽放比例
		
		if($scale < 1) {
		    $new_w = floor($scale*$im_w);
		    $new_h = floor($scale*$im_h);
		}
		$x = ($w-$new_w)/2;
		$y = ($h-$new_h)/2;
		/*
		...
		*/
		...
		if (function_exists ( "imagecopyresampled" )) {
			...
			imagealphablending($dst_im, false);
			imagesavealpha($dst_im, true);
			imagecopyresampled ( $dst_im, $image, $x, $y, 0, 0, $new_w, $new_h, $im_w, $im_h );
		} else {
			$dst_im = imagecreate ( $w, $h );
			imagealphablending($dst_im, false);
			imagesavealpha($dst_im, true);
			imagecopyresized ( $dst_im, $image, $x, $y, 0, 0, $new_w, $new_h, $im_w, $im_h );
		}
		header ( "content-type:$Type" );
		switch ($Type) {
			case "image/jpeg" :
